...
import { boolean, check, decimal, index, int, json, mysqlEnum, mysqlTable, timestamp, uniqueIndex, varchar, } from 'drizzle-orm/mysql-core';
...
export const cables = mysqlTable('cables', {
    ...
    uniqueIndex('cables_brand_color_mm_store_unique').on(table.brand, table.color, table.mm, table.storeId),
    ...
    check('cables_internal_code_range_check', sql `${table.internalCode} between 0 and 999999`),
    check('cables_mm_positive_check', sql `${table.mm} > 0`),
    check('cables_price_per_meter_positive_check', sql `${table.pricePerMeter} >= 0`),
]);
export const labels = mysqlTable('labels', {
    id: int('id', { unsigned: true }).autoincrement().primaryKey(),
    cableId: int('cable_id', { unsigned: true })
        .notNull()
        .references(() => cables.id, { onDelete: 'restrict', onUpdate: 'cascade' }),
    cableNameSnapshot: varchar('cable_name_snapshot', { length: 255 }).notNull(),
    internalCodeSnapshot: int('internal_code_snapshot', { unsigned: true }).notNull(),
